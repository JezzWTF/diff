...
export const FileDiffCard: React.FC<FileDiffCardProps> = ({ fileDiff }) => {
  ...
  const renderPath = () => {
    if (fileDiff.isNewFile) return <span className="text-green-400">{newPathDisplay}</span>;
    if (fileDiff.isDeletedFile) return <span className="text-red-400">{oldPathDisplay}</span>;
    if (fileDiff.isRenamed) return <> <span className="text-red-400 line-through">{oldPathDisplay}</span> <span className="text-gray-500 mx-1">&rarr;</span> <span className="text-green-400">{newPathDisplay}</span></>;
    return <span className="text-sky-400">{newPathDisplay}</span>; // Modified
  };
  ...
  return (
    <div className="bg-gray-800 rounded-lg shadow-lg overflow-hidden transition-all duration-300 ease-in-out">
      <header 
        className="flex items-center justify-between p-3 sm:p-4 bg-gray-700 bg-opacity-50 cursor-pointer hover:bg-gray-700 transition-colors"
      ...
      >
        ...
        <div className="flex items-center">
            ...
            {isCollapsed ? <ChevronDownIcon className="w-5 h-5 text-gray-400" /> : <ChevronUpIcon className="w-5 h-5 text-gray-400" />}
        </div>
      </header>
      ...
      {!isCollapsed && (
        <div className="p-0 sm:p-1 md:p-2">
          {fileDiff.headerLines && fileDiff.headerLines.length > 0 && (
            <div className="px-3 py-2 sm:px-4 sm:py-3 bg-gray-900 bg-opacity-30">
          ...
          {fileDiff.fileMetaLines && fileDiff.fileMetaLines.length > 0 && (
             <div className="px-3 py-2 sm:px-4 sm:py-3 bg-gray-800">
